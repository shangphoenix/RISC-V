...
void emuart_init(uint8_t uartNo,uint32_t baud)
{
    ...
    UART_NO = uartNo;
    if(uartNo==123) useremuart_frame('a',(uint8_t*)"a");//设置为永远不会运行，为了使得useremuart_frame不被优化掉，有静态变量无法动态命令化。
}
...
extern __attribute__((section (".component_list"))) void *  ComponentFun[MCU_SECTORSIZE/2];
//===========================================================================
//函数名称：emuart_frame
//功能概要：组帧，将待组帧的数据加入帧中。本函数应被放置于中断处理程序中。
//参数说明：ch：串口接收到一个字节数据；数组data：存放接收到的数据。
...
uint16_t useremuart_frame(uint8_t ch, uint8_t *data)
{
    ...
    if((length-1) != 0 && (index-1) >= length+7)
    {
        ...
        if((length-1) == 45)
        {
            ...
            {
                flash_read_physical((uint8_t*)imei,(uint32_t)ComponentFun[23],16);
