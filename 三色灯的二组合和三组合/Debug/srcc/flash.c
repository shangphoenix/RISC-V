...
//        N = N - (MCU_SECTORSIZE-offset);
//        uint8_t i = 1;
//        while(N >= 128)
//        {
//            flash_write(sect+i,0,128,(buf+(MCU_SECTORSIZE-offset)+128*(i-1)));
//            N = N - 128;
//            i++;
//        }
//        if((N > 0) && (N < 128))
//        {
//            flash_write(sect + i,0,N,(buf+(MCU_SECTORSIZE-offset)+128*(i-1)));
//        }
//        //（3.1.2）再写入第二个扇区
//    }
//    //（3.2）写入字节数不会跨扇区
...
////            flash_erase(sect);
//        //将数组写入扇区
//        flag = flash_Best(sect,0,MCU_SECTORSIZE,data);
//    }
//    //等待写入操作完成
//    while( (FLASH->STATR & FLASH_STATR_BSY) != 0U);
//
...
//    if (offset+N>=128)
//    {
//        flash_test(sect,offset,128-offset,data);
//        num = N-(128-offset);
//        uint8_t i = 1;
//        while(num>=128)
//        {
//            flash_test(sect + i,0, 128,(data + (128-offset) + (128 * (i - 1))));
//            i++;
//            num = num - 128;
//        }
//        if((num>0)&&(num<128))
//        {
//            flash_test(sect + i,0, num,(data + (128-offset) + (128 * (i - 1))));
//        }
//    }
//    else
//    {
//        flash_test(sect,offset,N,data);
//    }
//
//
//    return flag;
    //【20200124-修改】更改写入方式为快速写入
    uint8_t i;
...
//    uint32_t page_4K;
//    uint32_t FLASH_Page;
//    uint16_t WRP0 = 0xFFFF;
//
//    page_4K = 0x00000000;
//    temp = M/32;
//
//    page_4K |= (0x01<<temp);
//    FLASH_Page = (uint32_t)(~page_4K);
//    WRP0 = (uint16_t)(FLASH_Page & ((uint32_t)0x000000FF));
...
//        FLASH->MODEKEYR = FLASH_KEY2;
    ...
    //配置相关寄存器
//    FLASH->WPR = FLASH_WPR_WRP;
    //FLASH->WPR =FLASH_WRProt_Pages28to31;
...
//    address = (uint32_t)(sect * FLASH_PAGE_SIZE + FLASH_ADDR_START + offset);
//    addr = address;
//
////    /* 【1】【JCQ-20210511 】 这段代码 一键更新速度比较慢  这是单字节写入  */
////    //（4）循环写入双字，每8个数写一次
////    for(i = 0; i < N; i+=8)
////    {
////        //低位4字赋值
////        data_L = (uint16_t)((buf[i])|(buf[i+1]<<8));
////        flash_write_one(addr + i,data_L);
////        data_LL  = (uint16_t)((buf[i+2])|(buf[i+3]<<8));
////        flash_write_one(addr + i + 2,data_LL);
////        //高位4字赋值
////        data_H = (uint16_t)((buf[i+4])|(buf[i+5]<<8));
////        flash_write_one(addr + i + 4,data_H);
////        data_HH =  (uint16_t)((buf[i+6])|(buf[i+7]<<8));
////        flash_write_one(addr + i + 6,data_HH);
////    }
////    //（5）等待 FLASH_SR 寄存器中的 BSY 位清零，等待操作完成
////    while( (FLASH->STATR & STATR_BSY) != 0U);
////    return 0;
//
//    /* 【2】【JCQ-20210512】这段代码是为了解决一键更新速度慢所采用四字节写入法。 */
////    flash_init();                 //解锁快速编程模式
////    /* 闪存缓冲区负载 */
////    FLASH_bufreset();
////    for(i=0;i<N;i+=16)
...
////    return flag;
//
////    【3】  【JCQ-20210511】这个函数可以完成那个上面的操作，每次写入四个字节，
////        flash_write_fast(sect, N, buf);
//}
uint8_t flash_Best(uint16_t sect,uint16_t offset,uint16_t N,uint8_t *buf)
{
    uint32_t temp;
